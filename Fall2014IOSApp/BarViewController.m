...
-(void)loadResultWithDataArray:(NSArray *)resultArray {
    ...
    if ([self.tableViewDisplayDataArray count] > 0) {
        // 2
        [self.vocalizer speakString:[NSString stringWithFormat:/*@"I found %lu %@ restaurants"*/@"I found %lu %@",
                                     (unsigned long)[self.tableViewDisplayDataArray count],
                                     self.searchCriteria]];
    }
    
    else {
        [self.vocalizer speakString:[NSString stringWithFormat:/*@"I could not find any %@ restaurants"*/@"I could not find any %@",
                                     self.searchCriteria]];
    }
}
